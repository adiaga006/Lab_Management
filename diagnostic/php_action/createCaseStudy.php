...
try {
    if ($_POST) {	
        ...
        if ($result->num_rows > 0) {
            ...
            // Chèn case study mới
            $sql = "INSERT INTO case_study (case_study_id, case_name, location, categories_id, start_date, pond_id, status) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $connect->prepare($sql);
            if (!$stmt) throw new Exception("Prepare failed: " . $connect->error);

            $stmt->bind_param("sssissi", $caseStudyId, $caseName, $location, $categoryId, $startDate, $pondId, $status);
