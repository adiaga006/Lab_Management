...
<div class="page-wrapper">
    <div class="container-fluid">
                    ...
                    <div class="modal-body">
                        <form id="uploadForm" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" name="upload_date" id="uploadDatePicker" class="form-control" required>
                                <input type="hidden" name="case_study_id" value="<?php echo htmlspecialchars($caseStudyId); ?>">
                            </div>
                            <div class="form-group">
        ...
        <div class="form-inline mb-4">
            ...
            <button class="btn btn-danger" id="deleteSelected" style="display: none;" onclick="showConfirmDialog()">
        ...
        <div class="custom-confirm">
            ...
            <div class="custom-confirm-buttons">
                <button class="btn btn-secondary" onclick="closeConfirmDialog()">
                ...
                <div class="img-container" data-date="<?php echo $date; ?>">
                    <div class="date-header">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="custom-control custom-checkbox select-all-container" style="display: none;">
                                <input type="checkbox" class="custom-control-input select-all" 
                                       id="selectAll_<?php echo $date; ?>" 
                                       data-date="<?php echo $date; ?>">
                                <label class="custom-control-label" for="selectAll_<?php echo $date; ?>">Select All</label>
                            </div>
                        </div>
                    </div>
                    <div class="image-grid">
                        <?php foreach ($images as $image): ?>
                            <div class="image-item">
                                <div class="custom-control custom-checkbox image-checkbox" style="display: none;">
                                    <input type="checkbox" class="custom-control-input image-select" 
                                           id="<?php echo basename($image); ?>"
                                           data-date="<?php echo $date; ?>"
                                           data-path="<?php echo $image; ?>">
                                    <label class="custom-control-label" for="<?php echo basename($image); ?>"></label>
                                </div>
                                <a data-fancybox="gallery-<?php echo $date; ?>" 
                                   href="<?php echo $image; ?>"
...
<script>
    ...
    function updateDeleteButton() {
        ...
        deleteButton.style.display = selectedImages.length > 0 && deleteMode ? 'block' : 'none';
    }
    ...
    function toggleDeleteMode() {
        ...
        toggleBtn.textContent = deleteMode ? 'Cancel Delete' : 'Delete Images';
        
        // Show/hide checkboxes and select all
    ...
    function showConfirmDialog() {
        document.querySelector('.custom-confirm-backdrop').style.display = 'block';
        document.querySelector('.custom-confirm').style.display = 'block';
    }

    function closeConfirmDialog() {
        document.querySelector('.custom-confirm-backdrop').style.display = 'none';
    ...
    function deleteSelectedImages() {
        const selectedImages = document.querySelectorAll('.image-select:checked');
        const imagePaths = Array.from(selectedImages).map(checkbox => checkbox.dataset.path);
        ...
        .then(data => {
            if (data.success) {
                // Xóa các ảnh khỏi DOM thay vì reload trang
                data.deletedFiles.forEach(path => {
                    ...
                    if (checkbox) {
                        ...
                        setTimeout(() => {
                            imageItem.remove();
                            // Kiểm tra nếu không còn ảnh nào trong ngày đó
                            const dateContainer = imageItem.closest('.img-container');
                            if (dateContainer && !dateContainer.querySelector('.image-item')) {
                ...
                toggleDeleteMode(); // Tắt chế độ xóa
            } else {
                showToast(data.message || 'Error deleting images', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
